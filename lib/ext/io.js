    ...
    const followup = (msg, usr) => {
      if (usr === data.user) {
        ...
        db.run('INSERT OR REPLACE INTO storedmessages values(?, ?, ?, ?, ?)',
          [key, msg, images, usr.name, Date.now()], err => {
            usr.sendMessage(err ? 'A database error occurred.' : 'Success!');
          });
      }
    };
...
const unset = {
  ...
  exec: function unset (data) {
    ...
    db.run('DELETE FROM storedmessages WHERE key=?', [key], err => {
      data.user.sendMessage(err ? 'A database error occurred.' : 'Success!');
    });
  }
};
...
const search = {
  ...
  exec: function search (data) {
    ...
    db.all('SELECT * FROM storedmessages WHERE key LIKE ?', args,
      (err, rows) => {
        if (err || !rows.length) {
          data.user.sendMessage(err ?
        ...
        while (rows.length)
          data.user.sendMessage(`
            <table cellpadding="4">
              <tr><th>Key</th><th>Images</th><th>Uploader</th><th>Date</th></tr>
...
module.exports = {
  ...
  init: stumble => {
    ...
    db.run(`
      CREATE TABLE IF NOT EXISTS storedmessages(
        key TEXT UNIQUE,
        message TEXT,
        images INTEGER,
        uploader TEXT,
